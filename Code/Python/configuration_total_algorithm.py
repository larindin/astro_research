...
# Nonlinear least-squares algorithm parameters
thrusting_duration_cutoff = 5
thrusting_cutoff_offset = 10
additional_measurements = 5
first_magnitude_sampling_range = (1.08, 1.09)
second_magnitude_sampling_range = (0.999, 1.001)
# magnitudes = np.array([1.1, 1.2, 1.3])
initial_guess_magnitudes = np.array([1.1])

# Particle filter parameters
num_particles = 100
initial_weights = np.full(num_particles, 1/num_particles)
PF_measurement_covariance = measurement_noise_covariance * (10)**2
state_roughening_cov = np.eye(6)*1e-4**2
lambdar_roughening_cov = np.eye(3)*1e-4**2
angle_roughening_cov = np.eye(2)*np.deg2rad(1e-2)**2
magnitude_roughening_cov = np.eye(1)*1e-5**2
roughening_cov = scipy.linalg.block_diag(state_roughening_cov, lambdar_roughening_cov, angle_roughening_cov, magnitude_roughening_cov)
