...
horizon = len(filter_time) -1

smoothed_estimate_vals, smoothed_covariance_vals = run_EKF_smoothing(CR3BP_dynamics_equation_STM,
                                                                     dynamics_function_args,
                                                                     filter_time,
...
control_errors = []
control_covariances = []
smooth_control_errors = []
smooth_control_covariances = []
for run_index in range(num_runs):
    ...
    smooth_control_errors.append(smoothed_estimate_vals[6:9] - truth_control[:, -horizon-1:])
    smooth_control_covariances.append(smoothed_covariance_vals[6:9, 6:9])
...
smoothed_estimation_errors = compute_estimation_errors(truth_vals[:, -horizon-1:], [smoothed_estimate_vals], (0, 6))
smoothed_three_sigmas = compute_3sigmas([smoothed_covariance_vals], (0, 6))
smooth_control_3sigmas = compute_3sigmas(smooth_control_covariances, (0, 3))
...
plot_3sigma(time_vals, control_errors, control_3sigmas, "control", scale="linear", alpha=0.25)
...
plot_3sigma(time_vals[-horizon-1:], smoothed_estimation_errors, smoothed_three_sigmas, "position", scale="linear", alpha=0.25)
plot_3sigma(time_vals[-horizon-1:], smoothed_estimation_errors, smoothed_three_sigmas, "velocity", scale="linear", alpha=0.25)
plot_3sigma(time_vals[-horizon-1:], smooth_control_errors, smooth_control_3sigmas, "control", scale="linear", alpha=0.25)
# plot_3sigma(time_vals[-51:], smoothed_estimation_errors, smoothed_three_sigmas, "position", scale="log", alpha=0.25)
# plot_3sigma(time_vals[-51:], smoothed_estimation_errors, smoothed_three_sigmas, "velocity", scale="log", alpha=0.25)
