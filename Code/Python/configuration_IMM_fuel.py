...
initial_kernel_covariance = scipy.linalg.block_diag(initial_state_covariance, np.eye(6)*0.2**2)
initial_estimate = np.concatenate((np.random.default_rng(seed).multivariate_normal(initial_truth[0:6], initial_state_covariance), np.zeros(3), np.ones(3)*1e-9))
...
initial_mode_probabilities = np.array([1, 0])
mode_transition_matrix = np.array([[0.99, 0.01], [0.1, 0.9]])