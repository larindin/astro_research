...
for run_index in range(num_runs):
    ...
    state_size = np.size(truth_vals, 0)
    new_initial_estimates = np.empty((state_size, 2, kernels_per_mode))
    new_initial_covariances = np.empty((state_size, state_size, 2, kernels_per_mode))
    initial_kernel_probabilities = np.empty((2, kernels_per_mode))
    for mode_index in range(2):
        for kernel_index in range(kernels_per_mode):
            new_initial_estimates[:, mode_index, kernel_index] = truth_vals[:, 0]
            new_initial_covariances[:, :, mode_index, kernel_index] = scipy.linalg.block_diag(initial_state_covariance, initial_costate_covariance)
            initial_kernel_probabilities[mode_index, kernel_index] = initial_mode_probabilities[mode_index] / kernels_per_mode
    ...
    initial_estimates.append(new_initial_estimates)
    initial_covariances.append(new_initial_covariances)
...
GMIMM = GMIMM_filter(dynamics_functions,
...
results = GMIMM.run_MC(initial_estimates,
                     initial_covariances,
                     initial_kernel_probabilities,
                     time_vals,
                     measurements,
                     measurement_args)
...
kernel_probabilities = results.kernel_probabilities
...
for run_index in range(num_runs):
    for mode_index in range(2):
        for kernel_index in range(kernels_per_mode):
            ax.scatter(plot_time, kernel_probabilities[run_index][mode_index, kernel_index, :], alpha=0.5, s=4)
