...
from configuration_initial_filter_reformulated import *
from CR3BP import *
...
def coasting_dynamics_equation(t, X, mu, umax):
    ...
    jacobian = reformulated_min_energy_jacobian(state, costate, mu, umax)

    ddt_state = CR3BP_DEs(t, state, mu)
    ddt_costate = reformulated_min_energy_ODE(t, X, mu, umax)[6:12]
    # K = np.diag(np.ones(6)*20)
    # ddt_costate = -K @ costate
    ddt_STM = jacobian @ STM
...
def maneuvering_dynamics_equation(t, X, mu, umax):
    ...
    ddt_state = reformulated_min_energy_ODE(t, X, mu, umax)
    ddt_STM = jacobian @ STM
...
posterior_control = get_reformulated_min_energy_control(output_estimate_vals[6:12, :], umax)
