...
def CR3BP_dynamics_equation_accel(t, X, mu, umax):
    ddt_state = CR3BP_accel_DEs(0, X, mu)
    return ddt_state

def CR3BP_dynamics_equation_STM_accel(t, X, mu, umax):
...
for run_index in range(num_runs):
    ...
    initial_estimates.append(generator.multivariate_normal(truth_vals[0:6, 0], initial_state_covariance))
...
# Run detection UKF
detection_UKF = UKF(CR3BP_dynamics_equation,
                    dynamics_args,
...
detection_UKF_output = detection_UKF.run_MC(initial_estimates,
                                            initial_UKF_cov,
                                            time_vals,
                                            measurements,
                                            measurement_args)

detection_UKF_time = detection_UKF_output.t
detection_UKF_anterior_estimates = detection_UKF_output.anterior_estimates
detection_UKF_posterior_estimates = detection_UKF_output.posterior_estimates
detection_UKF_anterior_covariances = detection_UKF_output.anterior_covariances
detection_UKF_posterior_covariances = detection_UKF_output.posterior_covariances

maneuver_start_indices = run_maneuver_detection_alg_MC(CR3BP_dynamics_equation_STM,
                                                        dynamics_args,
                                                        time_vals,
                                                        detection_UKF_anterior_estimates,
                                                        detection_UKF_posterior_estimates,
                                                        detection_UKF_anterior_covariances,
                                                        detection_UKF_posterior_covariances,
                                                        horizon,
                                                        detection_threshold)

# Run accel UKF
initial_accel_UKF_estimates = []
for run_index in range(num_runs):
    initial_accel_UKF_estimates.append(np.concatenate((initial_estimates[run_index], np.zeros(3))))

accel_UKF = UKF(CR3BP_dynamics_equation_accel,
                dynamics_args,
                filter_measurement_function,
...
accel_UKF_results = accel_UKF.run_MC(initial_accel_UKF_estimates,
                                     initial_accel_UKF_cov,
                                     time_vals,
                                     measurements,
                                     measurement_args)

accel_UKF_time = accel_UKF_results.t
accel_UKF_posterior_estimates = accel_UKF_results.posterior_estimates
...
OCIMM_measurements = []
for run_index in range(num_runs):
    maneuver_start_index = int(maneuver_start_indices[run_index])
    ...
    initial_costate_guesses[run_index] *= np.linalg.norm(truth_vals[6:12, maneuver_start_index]) / np.linalg.norm(initial_costate_guesses[run_index])
    
    initial_OCIMM_estimates.append(np.concatenate((initial_guessing_state[0:6], initial_costate_guesses[run_index])))
...
truth_control = get_min_fuel_control(truth_vals[6:12, maneuver_start_index:], umax, truth_rho)
estimated_controls = []
control_errors = []
control_covariances = []
for run_index in range(num_runs):
...
estimation_errors = compute_estimation_errors(truth_vals[:, maneuver_start_index:], output_estimates, (0, 12))
...
lambda_ratios = np.empty((6, len(time_vals[maneuver_start_index:]), num_runs))
for run_index in range(num_runs):
