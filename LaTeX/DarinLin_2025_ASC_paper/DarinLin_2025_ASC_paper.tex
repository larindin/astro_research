...
\begin{align}
    \bm{f}^{(1)}(\bm{x}) &= \begin{bmatrix}
        \bm{g}(\bm{\xi}) \\
        -A \bm{\lambda}
    \end{bmatrix}, \quad A = aI_{6 \times 6}
\end{align}
...
\begin{align}
    u_\text{max} = \mu_k^{(2)} u_{\text{max}, 0}
\end{align}
...
\begin{algorithmic}[1]
    \BeginBox
    \State $u_\text{max} \gets \mu_k^{(2)} u_\text{max, 0}$
    \EndBox
\end{algorithmic}
...
We perform simulations for two scenarios: one with measurements available at all times, and another with an artificial observation gap from $t \in [15, 20]$ days to assess the ability of the OCIMM to predict control. Each scenario is ran with 100 Monte Carlo simulations, where each simulation differs in the initial state estimate ${}^+\hat{\bm{x}}_0$ and measurement noise values $\bm{w}_k^{(i)}$. The parameters of the OCIMM and acceleration IMM are included in Table \ref{tab:truth-parameters}.
...
\begin{table}
    ...
    \begin{tabular}{c|c|c}
        ...
        $\Pi$ & $\begin{bmatrix}
            ...
            0.01 & 0.99
        \end{bmatrix}$ & n.d. \\
        $Q^{(1)}$ & $\text{blkdiag}(10^{-30}I_6, 10^{-4}I_6)$ & n.d. \\
        $Q^{(2)}$ & $\text{blkdiag}(10^{-30}I_3, 10^{-18}I_3, 10^{-2}I_3, 10^{-4}I_3)$ & n.d.
    \end{tabular}
